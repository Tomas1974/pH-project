...
public class Productionunits
{
        ...
        Employees
    {
        get;
        set;
    } //Den bøvler. Enten var tallet ikke angivet, så forventes det at være int? eller også var det et interval 50-100 så var det en string. Nu er det konventeret til altid at være en streng.
...
public class NumberToStringConverter : JsonConverter<string>
{
    public override string Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        if (reader.TokenType == JsonTokenType.Number)
        {
            return
                reader.GetInt32()
                    .ToString(); // Or use GetDouble(), GetDecimal(), etc., depending on the expected number type
        }
